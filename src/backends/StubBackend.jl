...
CI/离线测试后端：零外部依赖，固定圆轨道。

# 用途
- 离线 CI：无需 SatelliteToolbox，主链仿真全绿
- 单元测试：确定性输出，便于断言
- Cloud Agent：首次启动无需下载大量包

# 轨道模型
- 圆轨道（e=0），固定高度 550 km（Starlink 轨道面）
- SGP4 路径：用 TLE 中的轨道根数做二体传播（忽略摄动）
- TEME→ECEF：简化 GMST 旋转，精度约 10 km（CI 场景足够）
- 经纬高：WGS84 椭球近似

# 不适合
- 生产仿真：无大气阻力、无 J2 摄动，位置误差随时间累积
- 高精度链路评估：用 JuliaSpaceBackend
"""
module StubBackend

using Dates

include(joinpath(@__DIR__, "OrbitBackend.jl"))
using .OrbitBackend
