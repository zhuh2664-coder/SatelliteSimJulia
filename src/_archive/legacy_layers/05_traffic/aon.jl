...
本文件实现流量层的核心功能：将 TrafficDemand（地面端点之间的业务需求）
按照网络层提供的路由结果（RoutePath）进行"全有全无"分配，
汇总出每个时隙、每条 ISL/GSL 链路上的负载样本（LinkLoadSample），
最终封装为 TrafficEvaluation。

# 算法说明：全有全无（All-or-Nothing）流量分配
#
# 全有全无（AON）分配是交通工程与网络流理论中最基础的分配策略。
# 其核心思想是：
#   对于每个流量需求（OD 对），将其全部流量分配到当前可用的最短路径上，
#   不考虑路径上链路的容量限制。如果路径不可达，则该需求的流量被完全丢弃。
#
# 数学表达：
#   设网络 G=(V,E)，每个需求 d_k 的流量为 f_k。
#   AON 分配的结果是：
#     x_e = Σ_k  f_k · δ(e ∈ P*_k)
#   其中 P*_k 是需求 d_k 的最短路径，δ(·) 是指示函数。
#
# 优点：计算简单，适合作为初始解或基准评估。
# 缺点：不考虑容量约束，会高估热门链路的负载（"过饱和"），
#       低估冗余路径的利用率。在实际系统中常作为迭代算法的第一步。

# 时索引评估流程（Time-indexed Evaluation）：
#   仿真时间被离散化为等间隔时隙 [t_0, t_1, t_2, ...]。
#   对每个时隙 t_i：
#     1. 筛选出在该时隙活跃的所有 TrafficDemand（start_elapsed_s <= t_i < end_elapsed_s）
#     2. 对每个活跃需求，基于当前时隙的网络拓扑和接入决策计算路由
#     3. 将承载流量按路径累加到各 GSL/ISL 链路
#     4. 记录分配结果和链路负载快照
#   由于卫星星座的动态性（轨道运动导致拓扑变化），同一需求在不同时隙的路由可能不同。

# 依赖模块
- core/orbit_layer/time.jl：SimulationTimeGrid、timeslot_offsets、time_count
- core/network_layer/links.jl：ISLPhysicalLinkSeries、ISLPhysicalLinkSample、GSLPhysicalLinkSample
- core/network_layer/access.jl：AccessDecisionTable、access_decisions_at
- core/network_layer/routing.jl：RouteRequest、RoutePath、route_path_at
"""
