...
# ════════════════════════════════════════════════════════════════
# SGP4 → 裸数组桥接（多重分派：propagate_to_ecef 接受 TLE 输入）
# ════════════════════════════════════════════════════════════════
# 这是打通"真实 TLE 仿真走裸数组主路径"的关键桥接。
# runner/precomposed 调 propagate_to_ecef 时不需改——多重分派自动选 SGP4 版本。

export propagate_to_ecef

"""
    propagate_to_ecef(tle_elements::Vector{TLEOrbitElementSet}, tspan; time_grid) -> Array{Float64,3}

SGP4 传播 + TEME→ECEF 转换，直接返回裸数组 (N×T×3) km。

这是裸数组主路径的 TLE 入口——让真实 TLE 星座能走和 Walker 设计星座
完全相同的下游管线（evaluate_isl_batch / generate_topology / 路由 / 指标）。

注意：SGP4 需要真实 epoch 来算 GMST 旋转，因此传入 `time_grid`（含 epoch），
而非裸 `tspan`。`tspan` 用于确定时间点，epoch 用于坐标转换。
"""
