...
### 时间复杂度
本实现使用二叉堆（`DataStructures.PriorityQueue`）选择最小距离节点，
时间复杂度为 O((V+E)·log V)，其中 V 是卫星数量、E 是可用 ISL 数量。
相比线性扫描的 O(V²)，在大规模星座（数千颗卫星，Starlink 级）下可扩展性显著更好。
...
function shortest_isl_path(
    ...
    # 已访问节点集合：已确定最短路径的节点
    visited = Set{Int}()
    # 优先队列（二叉堆）：以当前已知最短距离为优先级，O(log V) 取最小 / 更新键
    frontier = PriorityQueue{Int,Float64}()
    frontier[source_satellite_id] = 0.0

    # === Dijkstra 主循环 ===
    reached = false
    while !isempty(frontier)
        # 步骤 1: 弹出当前距离最小的节点（堆顶，O(log V)）
        current = popfirst!(frontier).first
        # 其最短距离已确定；若被更优路径先行确定（陈旧项），直接跳过
        current in visited && continue
        current_distance = distances[current]
        # 若当前节点即为目标节点，完成搜索
        if current == destination_satellite_id
            reached = true
            break
        end
        # 标记当前节点为已访问（其最短距离已确定）
