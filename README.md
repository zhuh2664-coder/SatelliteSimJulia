...
# 可选：Opt / Security；Viz / GMAT 另有独立开关
...
julia --project=envs/backends-integration scripts/benchmark_orbit_backends.jl
```

本轮本地验收结果：默认统一入口 `14 passed, 0 failed, 7 skipped`；nightly 后端筛选入口 `3 passed, 0 failed`；三后端端到端测试 `71/71`；根当前回归 `200/200`；`demo()` 七个步骤全部通过。三份 GitHub Actions 工作流已通过 YAML 静态解析和本地命令验证，但尚未在 GitHub 托管 runner 上实跑。

Orbit 还提供显式的可选后端入口：`propagate_with_backend(backend, elements, tspan)` 可保留 `OrbitResult` 元数据，`propagate_to_ecef(backend, elements, tspan)` 则返回主链所需的 ECEF km 裸数组。`OrbitBackendSpec`、`available_orbit_backends()` 和 session-local 注册表负责发现与配置。JuliaSpace 后端在包内独立实现 `:two_body`、`:j2`、`:j4` 的长期项传播方程，不调用原生 `SatelliteSimOrbit` 传播器；当前 12 星、4 时刻（最长 600 s）对标的最大绝对误差为 `5.46e-12 km`，门限为 `1 m`。两条实现共享 EGM2008 常数和 SatelliteToolbox 的 TEME→PEF 坐标变换，因此这是传播方程与后端契约的跨实现验证，不是相对高保真数值力模型的精度认证。
