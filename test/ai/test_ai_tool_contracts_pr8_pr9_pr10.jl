...
const STARLINK_TLE_CONTRACT = joinpath(@__DIR__, "..", "..", "data", "tle", "celestrak", "starlink_gp_latest.tle")
...
@testset "AI tool contracts for traffic/routing/orbit" begin
    ...
    @testset "traffic AON carries routing intent" begin
        ...
        if data["traffic_evaluation_ran"] == true
            ...
            @test data["traffic_fallback"] == true
            @test data["routing_evaluation_scope"] == "matrix_shortest_path_summary"
        end
    end

    @testset "SGP4 TLE contract" begin
        if isfile(STARLINK_TLE_CONTRACT)
            data = JSON.parse(SatelliteSimLab.execute_tool(
                "run_simulation",
                Dict{String,Any}(
            ...
            @info "SGP4 TLE fixture not found; schema contract assertions already covered" STARLINK_TLE_CONTRACT
            @test true
        end
    end
end
